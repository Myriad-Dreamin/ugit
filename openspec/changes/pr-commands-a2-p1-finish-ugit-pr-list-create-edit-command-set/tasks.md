...
- [ ] 1.1 Review the approved OpenSpec artifacts for "Finish ugit PR list/create/edit command set" and confirm the canonical request/PR title is `feat(pr/commands): Finish ugit PR list/create/edit command set`
- [ ] 1.2 Confirm the proposal is ready for pooled execution, the reusable worktree from `/home/kamiyoru/work/ts/ugit/.meow-team-worktrees/meow-N` can be claimed, and conventional-title metadata `feat(pr/commands)` stays separate from `branchPrefix` and change paths
...
- [ ] 2.2 Run validation and capture reviewer findings for "Finish ugit PR list/create/edit command set"
