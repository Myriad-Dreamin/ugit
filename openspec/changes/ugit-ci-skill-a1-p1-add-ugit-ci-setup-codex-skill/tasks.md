...
- The lane worktree mounts `.codex` as a separate read-only filesystem, so the
  ...
  `mkdir -p .codex/skills/ugit-ci-setup` or
  `./scripts/materialize-ugit-ci-skill.sh`. Git metadata for this worktree is
  also read-only, so the lane cannot stage or commit a workaround into
  `.git/worktrees/meow-1` either. Even `git add` fails with
  `Unable to create '.git/worktrees/meow-1/index.lock': Read-only file system`.
- The authored skill payload remains ready under `skills/ugit-ci-setup/`. Copy
  and verification steps for a writable checkout now live in
  `skills/ugit-ci-setup/MATERIALIZE.md`, and the one-step helper lives at
