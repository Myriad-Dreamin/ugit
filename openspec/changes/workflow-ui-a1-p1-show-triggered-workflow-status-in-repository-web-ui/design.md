...
- Preserve the canonical request title `feat(web/repository): show repository workflow status` and conventional-title metadata `feat(web/repository)` across the materialized artifacts.
...
- Fetch through an internal API route: rejected because the repository page is server-rendered already and can reuse server helpers without introducing an internal HTTP dependency.
...
- Render one summary card per branch with branch name, short commit hash, overall status badge, per-workflow rows when artifact data exists, and queued, started, and finished timestamps when present.
- Render a panel-level mixed status when the repository has multiple branch summaries whose normalized statuses are not all the same.
...
- Canonical request/PR title: `feat(web/repository): show repository workflow status`
- Conventional title metadata: `feat(web/repository)`
- Slash-delimited roadmap/topic scope stays in conventional-title metadata and does not alter the OpenSpec change path `workflow-ui-a1-p1-show-triggered-workflow-status-in-repository-web-ui`.
