...
- [ ] 1.1 Confirm the approved OpenSpec artifacts for "Cache workflow runs with
...
- [ ] 1.2 Keep conventional-title metadata `perf(workflow/runner)` separate from
...
- [ ] 2.1 Extract worktree lifecycle helpers from `lib/pr-runner/runner.ts` so
      pull-request CI keeps ephemeral detached worktrees and manual workflow
      runs get a dedicated managed lifecycle
- [ ] 2.2 Implement managed manual-run preparation for
...
- [ ] 2.3 Preserve cache usefulness by resetting tracked state per run without
...
- [ ] 3.1 Update README execution-model notes and add focused runner tests for
      manual worktree creation, reuse, tracked-state reset, stale-state
      recovery, and the absence of normal post-run removal
- [ ] 3.2 Run `pnpm fmt`, `pnpm fmt:check`, `pnpm lint`, `pnpm test`, and
      `pnpm build`
