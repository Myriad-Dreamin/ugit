# manual-pr-merge-a1-p1-add-manual-pr-merge-approval-flow Specification

## Purpose
Define the manual PR merge approval flow so successful CI leaves pull requests
in `passed`, server-derived readiness gates a repo-scoped merge action, GitHub
squash merges run only after approval, and the mirrored local base branch
realigns to the latest GitHub base commit after merge.
## Requirements
