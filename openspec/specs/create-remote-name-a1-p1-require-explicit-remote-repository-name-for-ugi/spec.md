# create-remote-name-a1-p1-require-explicit-remote-repository-name-for-ugi Specification

## Purpose
Define the `ugit create` contract for requiring an explicit remote repository
name, validating it as one safe path segment, and using that supplied identity
through remote setup, origin-conflict handling, documentation, and regression
coverage.
## Requirements
