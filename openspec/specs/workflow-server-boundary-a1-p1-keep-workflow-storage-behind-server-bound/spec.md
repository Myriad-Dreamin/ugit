# workflow-server-boundary-a1-p1-keep-workflow-storage-behind-server-bound Specification

## Purpose
Define the workflow list, detail, and log-read server-boundary behavior so
repository resolution and SQLite-backed workflow storage access stay on the
server and browser-facing workflow contracts never expose raw repository
filesystem paths.
## Requirements
